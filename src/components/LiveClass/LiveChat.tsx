    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-gray-700 flex items-center justify-between shrink-0">
        <div>
          ...
          <span className={`text-xs ${isConnected ? 'text-green-400' : 'text-red-400'}`}>{isConnected ? '● En línea' : '● Desconectado'}</span>
        </div>
      </div>
          ...
          messages.map((msg) => (
            <div key={msg.id} className="flex gap-2">
              {msg.userAvatar ? (
                ...
                <div className="w-8 h-8 rounded-full bg-yellow-500 flex items-center justify-center text-xs font-bold shrink-0">{msg.userName.charAt(0)}</div>
              )}
      ...
      <div className="p-4 border-t border-gray-700 shrink-0">
        <div className="flex gap-2">
          <input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Escribe un mensaje..." className="flex-1 px-3 py-2 bg-black/30 border border-gray-600 rounded-lg text-white placeholder-gray-500 text-sm focus:border-yellow-400 focus:outline-none" disabled={!isConnected} />
          <button onClick={handleSendMessage} disabled={!isConnected || !inputMessage.trim()} className="px-3 py-2 bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-600 text-black font-bold rounded-lg flex items-center justify-center transition">
            <Send size={16} />
          </button>
        </div>
      </div>

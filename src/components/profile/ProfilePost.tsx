...
export default function ProfilePosts() {
  ...
  const formatDateTime = (iso: string) => new Date(iso).toLocaleString('es-ES', { dateStyle: 'medium', timeStyle: 'short' });
  ...
  const toggleLikePost = async (postId: string) => {
    try {
      ...
      try {
        ...
        setPosts((prev) => prev.map((p: any) => (p.id === postId ? { ...p, likeCount: serverCount, likedByMe: true } : p)));
        return;
      } catch (err: any) {
        ...
        if (shouldTryUnlike) {
          ...
          setPosts((prev) => prev.map((p: any) => (p.id === postId ? { ...p, likeCount: serverCount, likedByMe: false } : p)));
          return;
        }
        throw err;
      }
      ...
      setPosts((prev) =>
        prev.map((p: any) =>
          p.id !== postId
            ...
            : {
                ...p,
                comments: (p.comments || []).map((c: any) => (c.id === commentId ? { ...c, likeCount: Math.max(0, (c.likeCount || 0) + delta), likedByMe: delta > 0 } : c)),
              }
        )
      );
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow-md animate-pulse">
  ...
  if (error) {
    return <div className="p-6 border border-red-200 dark:border-red-800 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 shadow-md">{error}</div>;
  }
        ...
        <div key={post.id} className="border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-shadow duration-200 overflow-hidden">
          ...
          <div className="px-6 pt-5 pb-3 flex justify-between items-start border-b border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-3">
              {post.user?.profilePicture ? <img src={post.user.profilePicture} alt={getDisplayName(post.user)} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-yellow-400 flex items-center justify-center text-sm font-bold text-black">{getInitials(post.user)}</div>}
            ...
            <button onClick={() => handleDeletePost(post.id)} className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 transition-colors p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="Eliminar publicación">
              🗑️
            </button>
          </div>
          ...
          <div className="px-6 py-3 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-6">
              ...
              <button onClick={() => toggleLikePost(post.id)} className={`flex items-center gap-2 transition-colors group ${(post as any).likedByMe ? 'text-red-500' : 'text-gray-600 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400'}`} aria-pressed={(post as any).likedByMe ? true : false} title={(post as any).likedByMe ? 'Quitar me gusta' : 'Me gusta'}>
                <span className="text-xl group-hover:scale-110 transition-transform">❤️</span>
                <span className="text-sm font-medium">{(post as any).likeCount ?? 0}</span>
              </button>

              {/* Botón de Comentarios */}
              <button onClick={() => toggleComments(post.id)} className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors group">
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/30 border-t border-gray-200 dark:border-gray-700">
              {post.comments && post.comments.length > 0 ? (
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Comentarios ({post.comments.length})</h4>
                    ...
                    <div key={comment.id} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <button onClick={() => toggleLikeComment(comment.id, post.id)} className={`mt-2 flex items-center gap-1 text-xs transition ${comment.likedByMe ? 'text-red-500' : 'text-gray-500 hover:text-red-500'}`} aria-pressed={comment.likedByMe ? true : false} title={comment.likedByMe ? 'Quitar me gusta' : 'Me gusta'}>

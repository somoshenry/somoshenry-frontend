...
export default function UserProfilePosts({ userId }: UserProfilePostsProps) {
  ...
  return (
    <div className="space-y-4 pb-6 w-full">
      {posts.map((post) => (
        <div key={post.id} className="bg-gray-100 dark:bg-gray-800 rounded-2xl shadow-md p-5 space-y-4 mx-auto" style={{ maxWidth: '480px', width: '100%' }}>
          {/* Cabecera del post */}
          ...
          {/* Contenido del post */}
          {post.content && <p className="text-gray-800 dark:text-gray-200 whitespace-pre-line bg-gray-200 dark:bg-gray-700 rounded-xl p-3">{post.content}</p>}
          ...
          <div className="flex items-center justify-between mt-2">
            <button onClick={() => toggleLike(post.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full transition ${(post as any).likedByMe ? 'bg-red-100 text-red-500 dark:bg-red-900/20 dark:text-red-400' : 'bg-gray-200 text-gray-600 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600'}`}>
              <span className="text-lg">{(post as any).likedByMe ? '❤️' : '🤍'}</span>
              <span className="text-sm font-medium">{(post as any).likeCount || 0}</span>
            </button>
            <button onClick={() => toggleComments(post.id)} className="text-sm text-gray-500 dark:text-gray-400 hover:text-yellow-500 dark:hover:text-yellow-400">

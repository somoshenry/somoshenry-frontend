...
const LoginForm = () => {
  ...
  return (
    <>
      ...
      <form className="shadow-Oscuro border-[#ffff00] mx-auto dark:bg-gray-100 flex w-11/12 max-w-md min-w-[400px] flex-col bg-gray-100 items-center rounded-xl border-t-4 m-8 shadow-2xl" onSubmit={submitHandel} noValidate>
        ...
        <div className="p-10 rounded-xl dark:bg-gray-200  bg-white w-full">
          <ImputGeneric id="email" label="Email" type="email" value={loginformstate.email} name="email" onChange={changeHandler} />
          {error.email && <div className="text-red-400 mb-3 text-xs">*{error.email}</div>}
          <ImputGeneric id="password" type="password" label="Contraseña" value={loginformstate.password} name="password" onChange={changeHandler} />
          {error.password && <div className="text-red-400 mb-1 text-xs">*{error.password}</div>}
          <div className="flex justify-end w-full mb-6">
            <Link href="/forgot-password" className="text-blue-500 text-sm duration-150 hover:scale-[1.02] hover:underline">
          ...
          <div className="flex justify-center w-full mt-4">
            <Link href="/register" className="text-blue-500 mb-2 text-sm duration-150 hover:scale-[1.02] hover:underline">
              ¿No tienes cuenta? Crear cuenta
            </Link>
          </div>
          <p className="text-center w-full text-black mb-2 text-xl"> - o -</p>
          <div className="flex justify-center w-full">
            <Link className="bg-white w-full text-center hover:shadow-black text-md mt-2 transform cursor-pointer rounded-lg py-1 text-black shadow-sm/30 duration-300 hover:scale-105 hover:rainbow-shadow-hover" href={`${process.env.NEXT_PUBLIC_API_URL}/auth/google`}>

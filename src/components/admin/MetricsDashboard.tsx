...
export default function MetricsDashboard() {
  ...
  const [activeView, setActiveView] = useState<'general' | 'subscriptions'>('general');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setActiveView('general')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeView === 'general' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>
            General
          </button>
          <button onClick={() => setActiveView('subscriptions')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${activeView === 'subscriptions' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>
            <DollarSign size={16} />
            Suscripciones
          </button>
        </div>
      ...
      {activeView === 'subscriptions' ? (
        ...
        <>
          {/* Selector de tiempo para vista general */}

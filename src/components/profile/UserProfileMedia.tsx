    ...
    <div className="w-full max-w-2xl mx-auto grid grid-cols-2 md:grid-cols-2 gap-4 pb-6">
      {mediaPosts.map((post) => {
        ...
        return (
          <div
            key={post.id}
            className="aspect-square rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-800 shadow-md hover:shadow-xl transition-shadow group relative"
          >
            {isVideo ? (
              <VideoPlayer src={url} className="w-full h-full" objectFit="cover" />
            ) : (

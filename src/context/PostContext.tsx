...
export function PostProvider({ children }: { children: ReactNode }) {
  ...
  const addComment = async (postId: string, text: string, parentId?: string) => {
    ...
    try {
      ...
      // Si es respuesta, actualizar el árbol anidado
      if (parentId) {
        setPosts((prev) =>
          prev.map((p) => {
  ...
  const likeComment = async (commentId: string) => {
    try {
      ...
      // Actualizar likes en comentarios anidados recursivamente
      const updateCommentLikes = (comments: any[]): any[] => {
        return comments.map((c) => {
          if (c.id === commentId) {
            return { ...c, likeCount: Math.max(0, (c.likeCount || 0) + delta), likedByMe: delta > 0 };
          }
          if (c.replies && c.replies.length > 0) {
  ...
  const deleteComment = async (commentId: string) => {
    try {
      ...
      // Eliminar comentario del árbol anidado recursivamente
      ...
      setPosts((prev) =>
        prev.map((p) => ({
          ...p,
          comments: removeComment(p.comments || []),
        }))
      );

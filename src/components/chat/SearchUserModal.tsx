...
export default function SearchUserModal({ isOpen, onClose, onSelectUser, currentUserId }: SearchUserModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-start justify-center pt-24 px-4" onClick={onClose}>
      ...
      <div className="relative bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col border border-gray-200/50 dark:border-gray-700/50" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="p-6 border-b border-gray-200/50 dark:border-gray-700/50">
          ...
          <div className="flex items-center bg-gray-100/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl px-5 py-3 border border-gray-200/50 dark:border-gray-700/50 shadow-sm hover:shadow-md transition-shadow">
            ...
            <input type="text" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} placeholder="Buscar usuarios por nombre o email..." className="flex-1 ml-3 bg-transparent focus:outline-none text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 font-medium" autoFocus />
            {searchQuery && (
              <button onClick={() => handleSearch('')} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors p-1 hover:bg-gray-200/50 dark:hover:bg-gray-700/50 rounded-full">
          ...
          {isSearching ? (
            ...
            <div className="space-y-3">
              {searchResults.map((user) => (
                <button key={user.id} onClick={() => handleSelectUser(user)} className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-100/80 dark:hover:bg-gray-800/80 backdrop-blur-sm transition-all text-left hover:shadow-lg hover:scale-[1.01] border border-gray-200/50 dark:border-gray-700/50 hover:border-yellow-400/50">
                  {user.profilePicture ? <img src={user.profilePicture} alt={getDisplayName(user)} className="w-12 h-12 rounded-full object-cover ring-2 ring-gray-200 dark:ring-gray-700" /> : <div className="w-12 h-12 rounded-full bg-yellow-400 flex items-center justify-center text-black font-bold text-lg shadow">{getInitials(user)}</div>}
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 dark:text-white truncate text-base">{getDisplayName(user)}</p>
        ...
        <div className="p-4 border-t border-gray-200/50 dark:border-gray-700/50 bg-gray-50/80 dark:bg-gray-800/50 backdrop-blur-sm rounded-b-3xl">
          <p className="text-xs text-gray-600 dark:text-gray-400 text-center">💡 Tip: Los chats grupales estarán disponibles próximamente</p>
        </div>
      </div>
    </div>
  );

...
export default function ReportedUsers() {
  ...
  const handleBanUser = async (userId: string) => {
    const result = await Swal.fire({
      title: "¡Peligro! Baneo de Usuario",
      text: "¿Estás seguro de que deseas banear a este usuario? Esta acción bloqueará su cuenta permanentemente.",
      icon: "error",
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "¡Baneado!",
        text: "Usuario baneado exitosamente y reportes resueltos",
      });
  ...
  const handleRejectReports = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: "¿Estás seguro de que deseas rechazar estos reportes?",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      cancelButtonColor: "#3085d6",
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "¡Rechazado!",
        text: "Reportes rechazados correctamente",
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Error al rechazar reportes",
      });
    } finally {
      setActionLoading(false);
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {groupedReports.length} usuarios con reportes pendientes
            </p>
          </div>
        </div>
      </div>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="shrink-0">
                      {item.user.profilePicture ? (
                        ...
                        <div className="w-12 h-12 rounded-full bg-linear-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg">
                          {getUserName(item.user).charAt(0).toUpperCase()}
                        </div>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className="shrink-0 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-lg transition-colors"
                  >
                    Ver Detalles
                  </button>
                </div>
              ...
              <div className="flex items-center gap-3">
                {selectedUser.user.profilePicture ? (
                  <img
                    ...
                    alt={getUserName(selectedUser.user)}
                  ...
                  <div className="w-16 h-16 rounded-full bg-linear-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-2xl">
                    {getUserName(selectedUser.user).charAt(0).toUpperCase()}
                  </div>
                )}
                <div>
            ...
            <div className="p-6 space-y-6">
                  ...
                  {selectedUser.reports.map((report) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <div>
                          <span className="inline-block px-3 py-1 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 text-sm font-medium rounded-full">
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors disabled:opacity-50 font-medium"
                >
                  <Ban size={18} />
                  {actionLoading ? "Procesando..." : "Banear Usuario"}
                </button>
                <button
                  onClick={handleRejectReports}
                  disabled={actionLoading}
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors disabled:opacity-50 font-medium"
                >
                  <Shield size={18} />

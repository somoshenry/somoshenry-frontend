    ...
    <div className="relative flex flex-col w-full md:mt-2 mt-3 mb-5 rounded-xl border border-gray-300 bg-white dark:bg-gray-200 shadow-sm p-4 overflow-hidden">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-start gap-3">
          {shouldShowInitials ? <div className="w-12 h-12 rounded-full bg-[#ffff00] flex items-center justify-center text-black font-bold text-lg shrink-0">{getInitial(nombre)}</div> : <img src={picture!} alt={nombre} className="w-12 h-12 rounded-full object-cover shrink-0" onError={() => setImgLoadError(true)} />}

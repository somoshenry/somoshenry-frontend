    ...
    <>
      <div
        className={`flex items-center h-10 grow max-w-xs md:max-w-md bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full border-2 transition-all duration-300 ease-in-out cursor-pointer shadow-sm hover:shadow-lg
            ${isFocused ? 'border-yellow-400 ring-4 ring-yellow-400/30 shadow-xl scale-105' : 'border-gray-200 dark:border-gray-700 hover:border-yellow-300 dark:hover:border-yellow-500'}`}
        onClick={handleFocus}
      >
        <div className={`pl-4 pr-3 transition-all duration-300 ${isFocused ? 'text-yellow-400 scale-110' : 'text-gray-400 dark:text-gray-500'}`}>
